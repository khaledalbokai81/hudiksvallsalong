  ...
  return <><SiteHeader /><main className={`booking-page ${step === 1 ? "booking-page-details" : `booking-page-active ${bookingStage === "service" ? "booking-page-service" : ""}`}`}>
    ...
    <section className="booking-flow" aria-label="Bokningsflöde" ref={bookingFlowRef}>
      ...
      {!submitted && step === 2 && <form className="booking-form booking-selection-form" onSubmit={submitBooking}>
        ...
        {bookingStage === "service" && <><fieldset className="booking-stage-panel booking-desktop-service"><legend>Välj behandling</legend><div className="booking-service-grid">{services.map((service, index) => <label className={`booking-service ${serviceId === service.id ? "selected" : ""}`} key={service.id}><input type="radio" name="service" value={service.id} checked={serviceId === service.id} onChange={() => { setServiceId(service.id); setDate(""); setTime(""); setBookingStage("date"); }} required /><i>{String(index + 1).padStart(2, "0")}</i><span><strong>{service.name}</strong><small>{service.duration}</small></span><b>{service.price}</b><em aria-hidden="true">→</em></label>)}</div></fieldset><div className="booking-mobile-service"><span className="booking-mobile-service-label">Behandling</span><div className={`booking-mobile-select ${serviceMenuOpen ? "open" : ""}`}><button className="booking-mobile-select-trigger" type="button" aria-haspopup="listbox" aria-expanded={serviceMenuOpen} onClick={() => setServiceMenuOpen((open) => !open)}><span><small>{selectedService ? "Vald behandling" : "Välj en behandling"}</small><strong>{selectedService?.name || "Tryck för att välja"}</strong></span>{selectedService && <b>{selectedService.price}</b>}<i aria-hidden="true">⌄</i></button>{serviceMenuOpen && <div className="booking-mobile-select-menu" role="listbox" aria-label="Välj behandling">{services.map((service, index) => <button type="button" role="option" aria-selected={serviceId === service.id} className={serviceId === service.id ? "selected" : ""} onClick={() => { setServiceId(service.id); setDate(""); setTime(""); setServiceMenuOpen(false); }} key={service.id}><i>{String(index + 1).padStart(2, "0")}</i><span><strong>{service.name}</strong><small>{service.duration}</small></span><b>{service.price}</b><em aria-hidden="true">{serviceId === service.id ? "✓" : ""}</em></button>)}</div>}</div>{selectedService && <div className="booking-mobile-service-summary"><span><small>Vald behandling</small><strong>{selectedService.name}</strong></span><span><small>Längd</small><strong>{selectedService.duration}</strong></span><span><small>Pris</small><strong>{selectedService.price}</strong></span></div>}<button className="button booking-mobile-service-next" type="button" disabled={!selectedService} onClick={() => setBookingStage("date")}>Välj och fortsätt <span aria-hidden="true">→</span></button></div></>}
