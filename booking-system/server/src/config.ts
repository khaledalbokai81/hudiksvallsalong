...
const envSchema = z
  .object({
    ...
    BUSINESS_SETTINGS_CACHE_TTL_MS: z.coerce
      .number()
      .int()
      .min(0)
      .default(process.env.NODE_ENV === "test" ? 0 : 10_000),
    ...
    API_BACKGROUND_WORKERS_ENABLED: z
      .enum(["true", "false"])
      .default("true")
      .transform((value) => value === "true"),
    ...
    RELIABILITY_CLEANUP_ENABLED: z
      .enum(["true", "false"])
      .default("true")
      .transform((value) => value === "true"),
    RELIABILITY_CLEANUP_INTERVAL_MS: z.coerce.number().int().positive().default(5 * 60 * 1000),
    RELIABILITY_CLEANUP_BATCH_SIZE: z.coerce.number().int().positive().max(500).default(100),
    TRUST_PROXY: z
      .enum(["true", "false"])
      .default("false")
      .transform((value) => value === "true"),
    ...
    REQUEST_LOG_SUCCESS_SAMPLE_RATE: z.coerce.number().min(0).max(1).default(0.05),
