...
export type BookingDocument = {
  ...
  reminderEmailStagesSent?: number[];
...
const bookingSchema = new Schema<BookingDocument>(
  {
    ...
    serviceDurationHours: { type: Number, min: 0.25, max: 12 },
    ...
    reminderEmailStagesSent: [{ type: Number, min: 1, max: 168 }],
...
bookingSchema.index({ reminderEmailStagesSent: 1, appointmentAt: 1 });
bookingSchema.index({ status: 1, emailVerified: 1, appointmentAt: 1 });
