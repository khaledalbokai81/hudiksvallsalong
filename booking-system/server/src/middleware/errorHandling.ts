...
export const requestLogging: RequestHandler = (req, res, next) => {
  ...
  res.on("finish", () => {
    ...
    const shouldLogRequest = shouldRecordRequest(req, res.statusCode);
    const sanitizedPath = sanitizeLoggedPath(req.originalUrl);
    ...
    if (shouldLogRequest && mongoose.connection.readyState === 1 && req.originalUrl.startsWith("/api")) {
...
function isPrivilegedPath(path: string) {
  return path.startsWith("/api/admin") || path.startsWith("/api/monitor");
}

function shouldSampleSuccessfulRequest() {
  return config.REQUEST_LOG_SUCCESS_SAMPLE_RATE > 0 && Math.random() < config.REQUEST_LOG_SUCCESS_SAMPLE_RATE;
}

function shouldRecordRequest(req: Parameters<RequestHandler>[0], statusCode: number) {
  if (!req.originalUrl.startsWith("/api")) {
    return statusCode >= 400;
  }

  if (statusCode >= 400 || isMutationMethod(req.method) || isPrivilegedPath(req.originalUrl)) {
    return true;
  }

  return shouldSampleSuccessfulRequest();
}
...
export const apiNotFoundHandler: RequestHandler = (req, _res, next) => {
  ...
  next(
    createHttpError(
      404,
      `API route ${req.method} ${sanitizedPath} was not found`,
      "API_ROUTE_NOT_FOUND"
    )
  );
};
...
export const errorHandler: ErrorRequestHandler = (error, _req, res, _next) => {
  ...
  if (normalizedError.statusCode >= 500 && mongoose.connection.readyState === 1) {
    void SystemEvent.create({
      ...
      requestId: res.locals.requestId,
      method: _req.method,
      path: sanitizeLoggedPath(_req.originalUrl),
