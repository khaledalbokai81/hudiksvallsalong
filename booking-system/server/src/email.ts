...
type WaitlistAvailableEmailInput = WaitlistJoinedEmailInput & {
  bookingUrl: string;
  expiresAt: string;
};
...
export async function sendWaitlistJoinedEmail({
  ...
  await transporter.sendMail({
    ...
    text: [
      `Hi ${name},`,
      "",
      `You are on the ${businessName} waitlist for ${serviceName} at ${appointmentLabel}.`,
      "We will email you if this time becomes available."
    ].join("\n"),
    html: `<div style="font-family: Arial, sans-serif; color: #172126; line-height: 1.6;"><h1>Waitlist confirmed</h1><p>Hi ${escapeHtml(name)},</p><p>You are on the <strong>${escapeHtml(businessName)}</strong> waitlist for <strong>${escapeHtml(serviceName)}</strong> at ${escapeHtml(appointmentLabel)}.</p><p>We will email you if this time becomes available.</p></div>`
  });
}
...
export async function sendWaitlistAvailableEmail({
  ...
  bookingUrl,
  expiresAt
}: WaitlistAvailableEmailInput) {
  await transporter.sendMail({
    from: config.MAIL_FROM,
    to,
    subject: `A waitlisted time is available for ${serviceName}`,
    text: [
      `Hi ${name},`,
      "",
      `${appointmentLabel} is available at ${businessName}.`,
      "This is a first-come offer; the time is not reserved until booking completes.",
      `Respond before ${expiresAt}: ${bookingUrl}`
    ].join("\n"),
    html: `<div style="font-family: Arial, sans-serif; color: #172126; line-height: 1.6;"><h1>A time is available</h1><p>Hi ${escapeHtml(name)},</p><p><strong>${escapeHtml(appointmentLabel)}</strong> is available for ${escapeHtml(serviceName)} at ${escapeHtml(businessName)}.</p><p>This is a first-come offer; the time is not reserved until booking completes.</p><p><a href="${escapeHtml(bookingUrl)}" style="display:inline-block;background:#172126;color:#fff;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:700;">Book this time</a></p><p>Respond before ${escapeHtml(expiresAt)}.</p></div>`
  });
}
