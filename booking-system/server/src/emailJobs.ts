...
const bookingReminderPayloadSchema = z.object({
  bookingId: z.string(),
  stageHours: z.number().int().min(1).max(168).optional(),
  to: z.string().email(),
  name: z.string(),
  businessName: z.string(),
  serviceName: z.string(),
...
const waitlistJoinedPayloadSchema = z.object({
  to: z.string().email(),
  name: z.string(),
  businessName: z.string(),
  serviceName: z.string(),
...
const waitlistAvailablePayloadSchema = waitlistJoinedPayloadSchema.extend({
...
async function sendEmailJob(job: EmailJobDocument) {
  ...
  if (job.type === "bookingReminder") {
    ...
    await Booking.updateOne({ _id: payload.bookingId }, {
      $set: { reminderEmailSentAt: new Date() },
      ...(payload.stageHours ? { $addToSet: { reminderEmailStagesSent: payload.stageHours } } : {})
    });
    return;
  }

  if (job.type === "waitlistJoined") {
    await sendWaitlistJoinedEmail(waitlistJoinedPayloadSchema.parse(job.payload));
    return;
  }

  if (job.type === "waitlistAvailable") {
    await sendWaitlistAvailableEmail(waitlistAvailablePayloadSchema.parse(job.payload));
    return;
  }
...
export async function enqueueEmailJob(input: EnqueueEmailJobInput) {
  const result = await EmailJob.findOneAndUpdate(
    ...
    { upsert: true, new: true, setDefaultsOnInsert: true, rawResult: true, session: input.session }
  );
