...
export function BookingPage() {
  ...
  const [waitlistBusySlot, setWaitlistBusySlot] = useState<string>();
  ...
  }, [form.serviceId, publicSettings.bookingRules.bookingWindowDays]);
  ...
  useEffect(() => {
    ...
    void getWaitlistOffer(waitlistToken)
      .then(({ offer }) => {
        setForm((current) => ({
          ...current,
          name: offer.name,
          ...
          appointmentAt: offer.slotStartAt,
          waitlistToken
        }));
        setStatus("success");
        setMessage("Your waitlisted time is available. Submit the booking before the offer expires.");
      })
      .catch((error) => {
        setStatus("error");
        setMessage(error instanceof Error ? error.message : "This waitlist offer is unavailable.");
      });
  }, [waitlistToken]);
  ...
  function updateField(field: keyof BookingInput, value: string) {
    setForm((current) => ({
      ...current,
      [field]: value,
      ...(field === "serviceId" ? { appointmentAt: "", waitlistToken: undefined } : {})
    }));
  }

  async function handleJoinWaitlist(slotStartAt: string) {
    if (!form.name.trim() || !form.email.trim() || (publicSettings.bookingRules.requirePhone && !form.phone.trim()) || !form.serviceId) {
      setStatus("error");
      setMessage(`Enter your name, email, ${publicSettings.bookingRules.requirePhone ? "phone, " : ""}and service before joining a waitlist.`);
      return;
    }

    setWaitlistBusySlot(slotStartAt);
    setMessage("");
    ...
    try {
      ...
      setStatus("error");
      setMessage(error instanceof Error ? error.message : "Could not join this waitlist.");
    } finally {
      setWaitlistBusySlot(undefined);
    }
  }
    ...
    <section className="mx-auto grid max-w-7xl gap-8 px-5 py-12 lg:grid-cols-[minmax(0,1fr)_420px] lg:px-8">
      <div>
        ...
        <form onSubmit={handleSubmit} className="mt-9 space-y-5 rounded-lg bg-white p-5 shadow-soft sm:p-7">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                required={publicSettings.bookingRules.requirePhone}
          ...
          <div>
            ...
            {availabilityLoading ? (
              ...
              <TimeSlotPicker
                ...
                disabled={status === "submitting" || Boolean(waitlistBusySlot)}
                onSelect={(slotStartAt) => updateField("appointmentAt", slotStartAt)}
                onJoinWaitlist={(slot) => void handleJoinWaitlist(slot.slotStartAt)}
              />
            )}
            <input
              value={form.appointmentAt}
          ...
          <label className="block">
            <span className="field-label">Notes{publicSettings.bookingRules.requireNotes ? " (required)" : ""}</span>
            <textarea
              className="field-input min-h-28 resize-y"
              value={form.notes}
              onChange={(event) => updateField("notes", event.target.value)}
              maxLength={500}
              required={publicSettings.bookingRules.requireNotes}
